...
class Modelseguimiento():
    ...
    def insertarSeguimiento(self,db,seguimiento):
        ...
        sql2="SELECT cantidad_dias FROM control WHERE id_control={}".format(seguimiento.id_control)
        ...
        if datos2[0]==0:
            ...
            values = (datos[0], seguimiento.id_tomate, seguimiento.id_control, seguimiento.estado, seguimiento.diagnostico, seguimiento.porcen_fiab, seguimiento.foto_hoja,seguimiento.fecha_ini,seguimiento.fecha_ini,datos2[0])
            try:
                cursor.execute(sql, values)
                db.connection.commit()
                return json.dumps({'status':True,'data':'Seguimiento Registrado Correctamente','datos':datos}) 
